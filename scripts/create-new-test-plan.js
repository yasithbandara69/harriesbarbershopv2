...
const PRICE_AMOUNT = 100n; // $1.00 AUD (Testing)

async function createNewTestPlan() {
    console.log(`Creating New TEST Plan ($1.00) for: ${OLD_PLAN_NAME}...`);
    ...
    try {
        ...
        const planBody = {
            ...
            object: {
                ...
                id: '#new_gold_test_plan',
                subscriptionPlanData: {
                    name: OLD_PLAN_NAME + ' (TESTING $1)', // Unique name
                    eligibleItemIds: [ITEM_ID], // Link to our billing item
                    subscriptionPlanVariations: [
                        {
                            type: 'SUBSCRIPTION_PLAN_VARIATION',
                            id: '#new_gold_test_plan_variation',
                            subscriptionPlanVariationData: {
                                name: OLD_PLAN_NAME + ' (TESTING $1)',
                                phases: [
                                    {
                                        cadence: 'MONTHLY',
                                        pricing: {
        ...
        if (newPlan) {
            console.log("SUCCESS! New TEST Plan Created.");
